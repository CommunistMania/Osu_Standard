class SliderBF:
    def __init__(self, x, y, r, t, hitrad, apprad, hitspeed, vel, number, game):
        ...
        self.xold = int(self.xold)
        self.yold = int(self.yold)
        ...
        self.Arrow = loadImage("Arrow.png")
    ...
    def on_draw(self):
        ...
        if self.miss:
            ...
            stroke(100, 100, 100)
            strokeWeight(115)
            line(self.xold, self.yold, self.x1, self.y1)
            noStroke()
    ...
    def on_update(self):
        ...
        if self.r > 0:
            if dist(self.x, self.y, self.x1, self.y1) > 2 + self.r:
                self.delete()
    
        if self.r < 0:
            if dist(self.x, self.y, self.x1, self.y1) < self.r - 2:
                self.delete() 
